...
| part  | what it is |
|-------|------------|
| RAM   | `sieve/state.yaml` on `main` |
| Clock | a GitHub push webhook refreshes Warehouse `sieve-state`, which turns each commit to `sieve/state.yaml` into Freight (a 15m poll is the backstop) |
...
- `euler-sieve-step` (with `stateFile: ./repo/sieve/state.yaml`) runs fetch (`yaml-parse`), decode, execute and cross-off (`compose-output`), then write-back (`yaml-update`).
...
Only commits that change `sieve/state.yaml` start a promotion (the Warehouse sets `includePaths: [sieve/state.yaml]`). Commits touching other files, and empty commits, do not.
...
- **A promotion failed.** Kargo won't auto-promote the same Freight again. Fix the cause (the Promotion's message in the Kargo UI says what failed), then either:
  ...
  - commit any change to `sieve/state.yaml`, for example a comment edit. The step reads `i` from the file, so the run carries on where it stopped:
    ```sh
    git pull --rebase origin main
    echo "# restart $(date -u +%FT%TZ)" >> sieve/state.yaml
